...
async function submitToAppsScript(file,date){
  ...
  document.body.appendChild(post);post.submit();setTimeout(()=>post.remove(),2500);return {requestId};
}
...
async function pollGitHubPath(started,date,status,index,total){
  ...
  while(Date.now()<deadline){
    attempt++;
    const wait=attempt<=3?3000:attempt<=8?5000:8000;
    status.textContent=`② 第 ${index}/${total} 張已送出，等待 GitHub 確認…（約 ${Math.ceil((deadline-Date.now())/1000)} 秒內）`;
    await sleep(wait);
    try{
      ...
      console.warn('GitHub confirmation poll failed',e);
    }
  }
  throw new Error('GITHUB_CONFIRM_TIMEOUT');
}
function card(file,i,done){
  const d=document.createElement('div');d.className='media-thumb';d.dataset.index=i;const url=URL.createObjectURL(file);
  d.innerHTML=`<img src="${url}" alt="待上傳照片"><button type="button" aria-label="移除照片" ${done?'disabled':''}>×</button><small>${done?'✓ 已完成':(i===0?'封面照片':`照片 ${i+1}`)}</small>`;return d;
}
function savePayload(form,paths){
  const d=new FormData(form);const payload={id:d.get('id'),title:d.get('title'),date:d.get('date'),category:d.get('category'),image:paths[0]||'',photos:paths.slice(1).join('\n'),desc:d.get('desc'),url:d.get('url'),home:d.get('home')==='on'};
  if(typeof window.__libraryGallerySave==='function')return window.__libraryGallerySave(payload);
  try{if(typeof upsert==='function'&&typeof resetForm==='function'){upsert('gallery',payload,'成果');resetForm('galleryForm');return true}}catch(e){console.error(e)}
  return false;
}
function init(){
  ...
  box.innerHTML=`<div class="media-title"><div><strong>活動照片</strong><p>可一次上傳大量活動照片；第一張自動作為封面。</p></div><span>最多 ${MAX_BATCH} 張</span></div><div class="media-service-ready"><strong>✓ GitHub 大量照片上傳模式已啟用</strong><p>系統會逐張送出並確認；若中途暫停，可再次按「儲存成果」從未完成的照片續傳。</p></div><label class="media-drop"><input id="galleryFiles" type="file" accept="${ACCEPT}" multiple><b>＋ 選擇照片</b><span>可一次選取多張 JPG、PNG、WebP</span></label><div id="galleryMediaStatus" class="media-status">尚未選擇照片</div><div id="galleryThumbs" class="media-thumbs"></div><div class="media-help">大量上傳流程：①送出 → ②GitHub確認 → ③保留已完成進度 → ④全部完成後寫入成果與 Firestore。</div>`;
  old?.after(box);
  let selected=[],uploaded=[];
  const input=$('#galleryFiles'),thumbs=$('#galleryThumbs'),status=$('#galleryMediaStatus');
  function render(){
    thumbs.innerHTML='';selected.forEach((f,i)=>thumbs.appendChild(card(f,i,i<uploaded.length)));
    if(!form.dataset.uploadStage){
      if(!selected.length)status.textContent='尚未選擇照片';
      else if(uploaded.length)status.textContent=`已完成 ${uploaded.length}/${selected.length} 張；再次按「儲存成果」可繼續。`;
      else status.textContent=`已選擇 ${selected.length} 張照片；第一張將作為封面。`;
    }
  }
  input.addEventListener('change',()=>{
    let files=[...input.files].filter(f=>ACCEPT.includes(f.type));
    if(files.length>MAX_BATCH){alert(`一次最多 ${MAX_BATCH} 張，已保留前 ${MAX_BATCH} 張。`);files=files.slice(0,MAX_BATCH)}
    const tooBig=files.filter(f=>f.size>MAX_FILE);if(tooBig.length)alert(`有 ${tooBig.length} 張照片超過 10 MB，請先縮小後再上傳。`);
    selected=files.filter(f=>f.size<=MAX_FILE);uploaded=[];seenPaths.clear();form.dataset.uploadStage='';render();
  });
  ...
  form.addEventListener('submit',async e=>{
    ...
    try{
      ...
      for(let i=uploaded.length;i<selected.length;i++){
        ...
        const result=await pollGitHubPath(started,date,status,i+1,selected.length);
        uploaded.push(result.path);
        status.textContent=`③ 已完成 ${uploaded.length}/${selected.length} 張：${result.path}`;
      ...
      setTimeout(()=>{form.dataset.uploadStage='';render()},3000);
    }catch(err){
      form.dataset.mediaUploading='0';form.dataset.uploadStage='';if(submit){submit.disabled=false;submit.textContent=original}
      const map={AUTH_REQUIRED:'請先登入管理員帳號。',UPLOAD_NOT_CONFIGURED:'安全上傳網址尚未設定。',GITHUB_CONFIRM_TIMEOUT:`GitHub 確認等待逾時；目前已完成 ${uploaded.length}/${selected.length} 張。請稍後再按「儲存成果」續傳。`,GITHUB_RATE_LIMIT:`GitHub 公開查詢暫時達到頻率上限；已完成 ${uploaded.length}/${selected.length} 張。請約 10～15 分鐘後再按「儲存成果」續傳。`,GITHUB_CHECK_FAILED:'暫時無法確認 GitHub 圖片狀態，已保留目前完成進度。',CMS_SAVE_FAILED:'照片已全部建立，但成果資料寫入 CMS 失敗。'};
