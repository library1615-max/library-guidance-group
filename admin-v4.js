...
function addTopPreview(){
 ...
 const a=document.createElement('a');a.id='frontPreviewBtn';a.className='top-preview-btn';a.href='index.html';a.target='_blank';a.rel='noopener';a.innerHTML='↗ 預覽前台';
 actions.prepend(a);
}
...
function addPublishCenter(){
 ...
 $('#runPublishCheck')?.addEventListener('click',()=>{renderPublishCenter();window.toast?.('已重新檢查網站資料')});
 renderPublishCenter();
}
...
function renderPublishCenter(){
 ...
 const checks=[
  {name:'已發布公告',value:news.length-drafts,ok:true,note:`另有 ${drafts} 則草稿`},
  {name:'空白連結',value:brokenLinks,ok:brokenLinks===0,note:brokenLinks?'建議補上網址或檔案連結':'連結資料完整'},
  {name:'缺少圖片',value:missingImages,ok:missingImages===0,note:missingImages?'活動或成果尚未設定圖片':'圖片欄位完整'},
  {name:'網站基本設定',value:missingSettings===0?'完整':`${missingSettings} 項未填`,ok:missingSettings===0,note:'檢查網站名稱、Hero 與 SEO 摘要'}
 ];
 ...
 if(upcoming){const arr=[...events].filter(e=>e.date).sort((a,b)=>a.date.localeCompare(b.date)).slice(0,4);upcoming.innerHTML=arr.length?arr.map(e=>`<div class="upcoming-row"><time>${safe(e.date)}</time><span class="region-chip">${safe(e.region||'活動')}</span><b>${safe(e.title||'未命名活動')}</b><span>${safe(e.place||'')}</span></div>`).join(''):'<p class="empty-note">目前沒有活動資料。</p>'}
}
...
function addLivePreview(formId,type){
 ...
 const render=()=>{
  const d=new FormData(form), box=old.querySelector('.preview-content'); if(!box)return;
...
function addFilters(){
 const newsHead=$('.view[data-view="news"] .panel-head');
 if(newsHead&&!$('#newsStatusFilter')){const s=document.createElement('select');s.id='newsStatusFilter';s.className='search-input compact-select';s.innerHTML='<option value="all">全部狀態</option><option value="published">已發布</option><option value="draft">草稿</option>';newsHead.append(s);s.addEventListener('change',filterNewsRows)}
 const eventHead=$('.view[data-view="events"] .panel-head');
 if(eventHead&&!$('#eventRegionFilter')){const s=document.createElement('select');s.id='eventRegionFilter';s.className='search-input compact-select';s.innerHTML='<option value="all">全部區域</option><option>中區</option><option>北區</option><option>南區</option>';eventHead.append(s);s.addEventListener('change',filterEventRows)}
}
...
function enhanceTables(){
 const observer=new MutationObserver(()=>{filterNewsRows();filterEventRows();renderPublishCenter()});
 ['newsTableBody','eventTableBody','resourceTableBody','galleryTableBody','linkTableBody'].forEach(id=>{const el=document.getElementById(id);if(el)observer.observe(el,{childList:true})});
}

function addUnsavedIndicator(){
 const top=$('.top-actions');if(!top||$('#unsavedBadge'))return;const badge=document.createElement('span');badge.id='unsavedBadge';badge.className='unsaved-badge';badge.textContent='尚未修改';top.insertBefore(badge,top.lastElementChild);
 let dirty=false;
 $$('form').forEach(f=>{f.addEventListener('input',()=>{dirty=true;badge.textContent='有未儲存變更';badge.classList.add('dirty')});f.addEventListener('submit',()=>setTimeout(()=>{dirty=false;badge.textContent='已儲存';badge.classList.remove('dirty')},50));f.addEventListener('reset',()=>{dirty=false;badge.textContent='尚未修改';badge.classList.remove('dirty')})});
 window.addEventListener('beforeunload',e=>{if(dirty){e.preventDefault();e.returnValue=''}});
}

function init(){addTopPreview();addPublishCenter();addLivePreview('newsForm','news');addLivePreview('eventForm','event');addLivePreview('resourceForm','resource');addLivePreview('galleryForm','gallery');addLivePreview('linkForm','link');addFilters();enhanceTables();addUnsavedIndicator();}
